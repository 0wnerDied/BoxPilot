...
首次运行说明。
首次启动 TUN 时，BoxPilot 会请求管理员授权，将服务宿主和当前 sing-box
内核复制到系统保护目录。此后启动、停止和切换配置不再重复请求权限；更新
应用或更换内核时仅需授权更新一次服务。退出 BoxPilot 会停止服务管理的
sing-box 进程，但保留空闲服务以供下次使用。可在“设置”中显式移除服务。
普通系统代理模式不会安装或调用特权服务。
